...
class HTTPServerLibrary:
    ...
    def __processRequest(self, requestHeader, requestBody):
        ...
        if METHOD == 'GET':
            if PATH == '/':
                return self.fileHandler.print_database()
            else:
                # Get 1 customer
                return self.fileHandler.find_customer(name)
        # Add customer
        elif METHOD == 'POST':
            return self.fileHandler.add_customer(name, requestBody)
        elif METHOD == 'DELETE':
            return self.fileHandler.delete_customer(name)
        elif METHOD == 'PATCH':
            return self.fileHandler.update_customer(name, requestBody)
